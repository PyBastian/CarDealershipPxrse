...
describe("catálogo", () => {
  ...
  it("combina búsqueda, marca, precio y transmisión", () => { const state = parseCatalogState({ q: "vivaro 2023", marca: "Opel", transmision: "MANUAL", precioMax: "12000000" }); expect(filterVehicles(inventoryVehicles, state).map((v) => v.model)).toEqual(["Vivaro"]); expect(activeFilterCount(state)).toBe(4); });
  it("ordena por precio", () => { const state = parseCatalogState({ orden: "price-asc" }); const result = filterVehicles(inventoryVehicles, state); expect(result.map((vehicle) => vehicle.model)).toEqual(["Vivaro", "3008"]); });
  ...
  it("elige relacionados sin incluir el actual", () => { const current = inventoryVehicles.find((vehicle) => vehicle.make === "Toyota")!; const result = relatedVehicles(current, inventoryVehicles); expect(result.map((vehicle) => vehicle.model)).toEqual(["Vivaro"]); expect(result.some((v) => v.id === current.id)).toBe(false); });
});

...
test("desktop catalog search, sort, detail and gallery", async ({ page }, testInfo) => {
  ...
  await page.getByLabel("Buscar autos").fill("Vivaro");
  await page.getByRole("button", { name: "Buscar" }).click();
  await expect(page.getByText("Opel Vivaro").first()).toBeVisible();
  await page.getByLabel("Ordenar autos").last().selectOption("price-asc");
  await page.getByRole("link", { name: /Ver 2023 Opel Vivaro/ }).click();
...
test("mobile filters, vehicle detail and sticky CTA have no overflow", async ({ page }, testInfo) => {
  ...
  await page.locator('.filter-dialog select[name="marca"]').selectOption("Opel");

  ...
  return <>
    <section className="hero shell">
      <div className="hero-copy"><span className="eyebrow">Colección seleccionada</span><h1>Encuentra tu próximo auto.</h1><p>Una selección acotada, información clara y contacto directo. Sin ruido.</p>
        <form action="/autos" className="hero-search"><Search/><input name="q" placeholder="Buscar marca o modelo" aria-label="Buscar marca o modelo"/><button className="button">Buscar</button></form>
        <Link className="text-link" href="/autos">Ver todos los autos <ArrowRight size={17}/></Link>
      </div>
      <Link href={`/autos/${vehicles[0]?.slug ?? ""}`} className="hero-visual"><Image src="/vehicles/suv-graphite.png" alt="SUV grafito en estudio oscuro" fill preload sizes="(max-width: 800px) 100vw, 56vw"/><span><small>Destacado</small>{vehicles[0]?.year} {vehicles[0]?.make} {vehicles[0]?.model}<ArrowRight/></span></Link>
    </section>

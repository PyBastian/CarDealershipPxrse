...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="admin-shell"><aside className="admin-nav"><Link href="/admin" className="brand"><span className="brand-mark">LP</span>LO PRADO</Link><nav><Link href="/admin"><LayoutDashboard/> Resumen</Link><Link href="/admin/autos"><Car/> Inventario</Link><Link href="/admin/autos/nuevo"><Plus/> Nuevo auto</Link><Link href="/admin/configuracion"><Settings/> Configuración</Link></nav><Link href="/">Ver sitio público ↗</Link></aside><div className="admin-main">{children}</div></div>;
}

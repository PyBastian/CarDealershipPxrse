...
import { activeFilterCount, parseCatalogState } from "@/lib/catalog";
...
function chipUrl(params: Params, remove: string) {
  ...
  Object.entries(params).forEach(([key, value]) => { if (key !== remove && value) next.set(key, Array.isArray(value) ? value[0] : value); });
...
export default async function CatalogPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  const [inventory, vehicles] = await Promise.all([getPublicVehicles(), getPublicVehicles(state)]);
  ...
  return <div className="shell catalog-page">
    ...
    <div className="catalog-layout">
      ...
      <div className="catalog-results">
        <div className="results-head"><span>{vehicles.length} {vehicles.length === 1 ? "vehículo" : "vehículos"}</span><form>{Object.entries(raw).map(([key, value]) => key !== "orden" && value && <input key={key} type="hidden" name={key} value={Array.isArray(value) ? value[0] : value}/>)}<label>Ordenar por <SortSelect value={state.orden}/></label></form></div>
